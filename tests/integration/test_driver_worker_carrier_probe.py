...
class DriverWorkerCarrierProbeTests(unittest.TestCase):
    ...
    def test_finalize_changes_hash_and_is_idempotent_for_same_artifact(self) -> None:
        state = ProductionCarrierState.placeholder("calibration-v1", "a" * 64)

        finalized = state.finalize(b"portable-artifact-v1")
        finalized_again = finalized.finalize(b"portable-artifact-v1")
        ...
        self.assertEqual(b"portable-artifact-v1", finalized.artifact_bytes)

...
class BatchExecutionWrapper:
    """Serializable Daft carrier that invokes one kernel per physical batch."""
    ...
    _trace_min: int | None = field(default=None, init=False, repr=False, compare=False)
    _trace_max: int | None = field(default=None, init=False, repr=False, compare=False)
    _trace_registered: bool = field(default=False, init=False, repr=False, compare=False)
    ...
    def __getstate__(self) -> dict[str, Any]:
        ...
        state.update(
            ...
            _serialization_version=BATCH_WRAPPER_SERIALIZATION_VERSION,
        )
        return state

    def __setstate__(self, state: dict[str, Any]) -> None:
        version = state.get("_serialization_version") if type(state) is dict else None
    ...
    def _kernel_kind(self) -> str:
        return self.batch_kernel.kind if self.batch_kernel is not None else "scalar_envelope"
    ...
    def _record_trace(self, row_count: int) -> None:
        ...
        self._trace_min = row_count if self._trace_min is None else min(self._trace_min, row_count)
        self._trace_max = row_count if self._trace_max is None else max(self._trace_max, row_count)
    ...
    def __call__(self, receiver: Any, *args: Any, **kwargs: Any) -> list[Any]:
        ...
        keyword_columns = {name: _series_values(value) for name, value in kwargs.items()}
        lengths = {
            len(values)
        ...
        if (
            ...
            values = positional_columns[0]
            if any(value is None for value in values):
                raise ValueError("batch_kernel_null_input_unsupported")
            try:
                output = self.batch_kernel.invoke(values)
            except Exception:
                if not self.batch_kernel.fallback_on_error:
                    raise
            else:
                if len(output) != row_count:
                    raise ValueError("batch_kernel_output_length_mismatch")
                return output

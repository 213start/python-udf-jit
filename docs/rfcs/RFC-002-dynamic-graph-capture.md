...
**本次修订：** 记录标量实现状态与 Python 3.14/3.11 版本边界
...
RFC-002 的 CPython 3.14 捕获前端已进入生产代码：版本化字节码解码器、控制流图、异常表和位置表校验、抽象解释、源码映射、图中断、代码/依赖身份以及作业级有界捕获缓存均已实现。捕获阶段不执行用户函数；不透明语义形成带活跃值和恢复身份的 Python 区域，证明不足时在进入编译区域前保持整函数解释。

生产目标 Python 3.11.6 需要独立的字节码解码适配，当前 3.14 证据不外推为 3.11 资格。生成器、协程、反射、元类和任意 C 扩展内联仍不在本期范围内。
